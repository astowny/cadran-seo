...
Landing page for a **fictional** service: SEO optimisation of WordPress sites for
small local businesses in France. Written as a conversion-copywriting sample —
value proposition, PAS problem framing, four-step method, proof block with
figures, three-tier pricing, objection handling, and a lead form that now
...
|---|---|---|
| `cadran-seo` | `nginx:alpine` | serves `index.html`, `en/index.html`, `checkout.js`; proxies `/api/` |
| `cadran-seo-api` | Bun 1.3 | `POST /api/checkout`, `POST /api/lead`, `POST /api/stripe-webhook`, `GET /api/healthz` |
...
`cadran-seo` and `transvase` share no package and will not get one. Sharing is
...
projects even though its `container_name` is `cadran-seo-api`: `nginx.conf`
...
`docker inspect cadran-seo-api`. That is unavoidable with compose — environment
...
curl https://api.stripe.com/v1/webhook_endpoints \
  -u "$STRIPE_SECRET_KEY:" \
  --data-urlencode "url=https://cadran-seo.devanchor.company/api/stripe-webhook" \
  -d "enabled_events[]=payment_intent.succeeded" \
  -d "enabled_events[]=payment_intent.payment_failed" \
  -d "description=cadran-seo test-mode purchase notifications"
```
...
```
curl -sI https://cadran-seo.devanchor.company/en      # 301 -> /en/
curl -sI https://cadran-seo.devanchor.company/        # 200 + Cache-Control: no-cache
curl -s  https://cadran-seo.devanchor.company/api/healthz   # ok
docker logs cadran-seo-api | head -1                  # fingerprint line
```
...
- Fonts actually loaded (`fonts.googleapis.com`, verified 31/08/2026):
  **Instrument Serif** (display and headings), **Archivo** (UI and body),
  **JetBrains Mono** (data, labels, small caps). An earlier version of this
  README named Bricolage Grotesque and Newsreader; the page has never loaded
  them.
